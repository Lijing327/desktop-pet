<template>
  <!-- 方向翻转层：follow/wait 时跟随光标方向，idle 时随机看向 -->
  <div class="pet-flip" :class="{ 'flip-left': !facingRight }">
    <div class="pet-wrap" :class="[animClass, { 'pet-hovered': isHovered }]">
      ...
      <!-- 星星特效（follow / react） -->
      <Transition name="pet-fade">
        <div v-if="state === 'follow' || state === 'react'" class="sparkles-wrap" aria-hidden="true">
...
const props = defineProps<{
  ...
  /** hover 时施加发光效果 */
  isHovered?: boolean
  /** 面向方向：true = 右（默认），false = 左（翻转） */
  facingRight?: boolean
}>()

// ── 面部类型 ──────────────────────────────────────────────────────────────
type FaceType = 'normal' | 'happy' | 'sleep' | 'wide'

const FACE_MAP: Record<PetState, FaceType> = {
  idle:   'normal',
  follow: 'happy',
  wait:   'normal',
  sleep:  'sleep',
...
const faceType = computed((): FaceType => FACE_MAP[props.state])

// ── 动画类名（animOverride 优先于状态机映射）────────────────────────────
const STATE_ANIM: Record<PetState, string> = {
  idle:   'anim-breathe',
  follow: 'anim-walk',     // 跑步上下弹跳
  wait:   'anim-breathe',  // 原地呼吸等待
  sleep:  'anim-sleep',    // 倾斜慢呼吸
  drag:   'anim-breathe',  // 被拖时保持自然
  react:  'anim-bounce',   // 点击弹跳反馈
  remind: 'anim-shake',    // 提醒抖动
}

const animClass = computed(() => props.animOverride ?? STATE_ANIM[props.state])

// ── 朝向（外部 prop 控制） ────────────────────────────────────────────────
const facingRight = computed(() => props.facingRight !== false)

// ── 眨眼（仅 idle / wait 状态周期触发） ──────────────────────────────────
const isBlinking = ref(false)
let blinkTimer: ReturnType<typeof setInterval> | null = null

function scheduleBlink(): void {
  blinkTimer = setInterval(() => {
    if (props.state !== 'idle' && props.state !== 'wait') return
